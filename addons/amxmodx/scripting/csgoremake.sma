...
stock color_print( const player_id, const lang_formatting[], any:... )
{
    ...
    if( player_id )
    {
        ...
        else
        {
            ...
            client_print( player_id, print_chat, "%s", formatted_message );
        }
    }
    else
    {
...
public plugin_init()
{
	...
	register_dictionary_colored("csgoremake.txt");
	...
	g_Vault = nvault_open("csgoremake");
	if (g_Vault == -1)
	{
		set_fail_state("[CSGO Remake] Could not open file csgoremake.vault .");
		return 0;
	}
	else
	{
		log_amx("[CSGO Remake] File csgoremake.vault was successfully loaded.");
	}
...
public plugin_precache()
{
	...
	if (!fp)
	{
		set_fail_state("[CSGO Remake] Could not open file csgoremake.ini .");
		return 0;
	}
	else
	{
		log_amx("[CSGO Remake] File csgoremake.ini was successfully loaded.");
	}
	new buff[128];
	new section;
	new leftpart[48];
			...
			switch (section)
			{
				...
				case 2:
				{
					if (buff[0] != 91)
					{
						...
						{
							new error[128];
							formatex(error, 127 + strlen(rightpart), "[CSGO Remake] File ^"%s^" from section [DEFAULT] in csgoremake.ini is missing.", rightpart);
						...
						{
							new error[128];
							formatex(error, 127 + strlen(weaponP), "[CSGO Remake] File ^"%s^" from section [DEFAULT] in csgoremake.ini is missing.", weaponP);
							set_fail_state(error);
							return 0;
						}
						...
						//{
						//	new error[128];
						//	formatex(error, 127 + strlen(weaponW), "[CSGO Remake] File ^"%s^" from section [DEFAULT] in csgoremake.ini is missing.", weaponW);
						//	set_fail_state(error);
						//	return 0;
						//}
					}
				...
				case 3:
				{
					if (buff[0] != 91)
					{
						...
						{
							new error[128];
							formatex(error, 127 + strlen(weaponmodel), "[CSGO Remake] File ^"%s^" from section [SKINS] in csgoremake.ini is missing.", weaponmodel);
							set_fail_state(error);
							return 0;
						}
						...
						{
							new error[128];
							formatex(error, 127 + strlen(weaponP), "[CSGO Remake] File ^"%s^" from section [SKINS] in csgoremake.ini is missing.", weaponP);
							set_fail_state(error);
							return 0;
						}
						...
						//{
						//	new error[128];
						//	formatex(error, 127 + strlen(weaponW), "[CSGO Remake] File ^"%s^" from section [SKINS] in csgoremake.ini is missing.", weaponW);
						//	set_fail_state(error);
						//	return 0;
						//}
						switch (weapontype[0])
...
public plugin_cfg()
{
	...
	server_cmd("exec %s/csgoremake.cfg", g_szCfgDir);
...
public plugin_natives()
{
	...
	formatex(g_szConfigFile, 47, "%s/csgoremake.ini", g_szCfgDir);
...
public task_Info(task)
{
	...
	if (0 < id && 32 >= id)
	{
		color_print(id, "^4*^1 Joci ^4%s^1 v. ^3%s^1 creat de ^4%s", "CSGO Remake", "1Gx", "Nubo");
	}
	return 0;
}
...
_InfoPlugin()
{
	new grelease[33] = "playtest-20210322";
	new version[33] = "v0.3-dev";
	new author[128] = "https://www.github.com/kuamquat940";
	new rurl[128] = "https://www.github.com/kuamquat940/csgoremake/releases";
	server_print("");
	...
	server_print("[*] Keep plugin updated: %s", rurl);
	server_print("---------------------------------");
	server_print("");
	return 0;
}
...
public ev_NewRound()
{
	...
	if (!IsHalf() && !IsLastRound() && 0 < g_iRoundNum)
	{
		...
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_COMPETITIVE_INFO", g_iRoundNum, szNextMap);
	}
	if (IsLastRound())
	{
		set_pcvar_num(p_Freezetime, 10);
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_MAP_END");
		_ShowBestPlayers();
		set_task(7.00, "task_Map_End", 0, "", 0, "", 0);
	}	
	if (IsHalf() && !g_bTeamSwap)
	{
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_HALF");
		_ShowBestPlayers();
		new Float:delay = 0.0;
		new i = 1;
...
_ShowBestPlayers()
{
	...
	{
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_BEST_T", g_szName[BestPlayer], BestMVP, bonus);
	}
	else
	{
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_ZERO_MVP", "Terrorist");
	}
	...
	if (BestPlayer && BestPlayer <= g_iMaxPlayers)
	{
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_BEST_CT", g_szName[BestPlayer], BestMVP, bonus);
	}
	else
	{
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_ZERO_MVP", "Counter-Terrorist");
	}
	if (g_bLogged[BestPlayer])
	{
		g_iUserPoints[BestPlayer] += bonus;
...
public task_Team_Swap()
{
	...
	color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_RESTART");
	...
	switch (g_iMVPMsgType)
	{
		case 0:
		{
			switch (event)
			{
				case 0:
				{
					color_print(0, "^4%s^1 Round MVP: ^3%s^1 %L: ^4%d", "[CSGO Remake]", g_szName[id], -1, "CSGOR_MOST_KILL", g_iRoundKills[id]);
				}
				case 1:
				{
					color_print(0, "^4%s^1 Round MVP: ^3%s^1 %L", "[CSGO Remake]", g_szName[id], -1, "CSGOR_PLANTING");
				}
				case 2:
				{
					color_print(0, "^4%s^1 Round MVP: ^3%s^1 %L", "[CSGO Remake]", g_szName[id], -1, "CSGOR_DEFUSING");
				}
				default:
				{
				}
...
_GiveBonus(id, type)
{
	if (!g_bLogged[id])
	{
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_REGISTER");
		return 0;
	}
	new rpoints;
	switch (type)
...
public fw_FM_ClientUserInfoChanged(id)
{
	...
	if (szOldName[0])
	{
		...
		if (!equal(szOldName, szNewName, 0) && !g_IsChangeAllowed[id])
		{
			...
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_CANT_CHANGE_ACC");
			return 0;
		}
	}
	return 0;
...
public clcmd_say_reg(id)
{
	...
	{
		_ShowRegMenu(id);
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_MUST_LOGIN");
	}
	return 0;
}
...
_ShowRegMenu(id)
{
	...
	{
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_REG_CLOSED");
		return 0;
	}
	new temp[64];
...
public reg_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		case 1:
		{
			if (g_bLogged[id] != true)
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_REG_INSERT_PASS", 6);
		...
		{
			g_bLogged[id] = false;
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_LOGOUT_SUCCESS");
		}
		case 3:
		{
			_LoadData(id);
			...
			if (!equal(g_szUserPassword[id], g_szUserSavedPass[id], spLen))
			{
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_PASS_FAIL", g_iUserPassFail[id], 3);
					_ShowRegMenu(id);
				}
			}
			else
			{
				g_bLogged[id] = true;
				_ShowMainMenu(id);
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_LOGIN_SUCCESS");
			}
		}
		case 4:
		{
			...
			_SaveData(id);
			_ShowRegMenu(id);
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_REG_SUCCESS", g_szUserSavedPass[id]);
		}
		default:
		{
		}
...
public concmd_password(id)
{
	if (g_bLogged[id] == true)
	{
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_ALREADY_LOGIN");
		return 1;
	}
	new data[32];
	read_args(data, 31);
	...
	switch (index)
	{
		...
		default:
		{
			...
			if (!SameSkin)
			{
				...
				_SaveData(id);
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_SELECT_SKIN", sName);
			}
			else
			{
				g_iUserSelectedSkin[id][wid] = -1;
				_SaveData(id);
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_DESELECT_SKIN", sName);
			}
			_ShowSkinMenu(id);
		}
	}
...
public oc_craft_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		case 0:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_OPEN_NOT_ENOUGH");
				_ShowOpenCaseCraftMenu(id);
			}
			else
			{
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_DONT_SPAM", 5);
					_ShowOpenCaseCraftMenu(id);
					return 0;
				}
				_OpenCase(id);
		...
		case 1:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_CRAFT_NOT_ENOUGH", g_iCraftCost - g_iUserDusts[id]);
				_ShowOpenCaseCraftMenu(id);
			}
			else
			{
		...
		case 2:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_NOT_ENOUGH_POINTS", g_iKeyPrice - g_iUserPoints[id]);
				_ShowOpenCaseCraftMenu(id);
			}
			else
			{
				...
				g_iUserKeys[id]++;
				_SaveData(id);
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_BUY_KEY");
				_ShowOpenCaseCraftMenu(id);
			}
		}
		case 3:
		{
			if (1 > g_iUserKeys[id])
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_NONE_KEYS");
				_ShowOpenCaseCraftMenu(id);
			}
			else
			{
				...
				g_iUserKeys[id]--;
				_SaveData(id);
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_SELL_KEY");
				_ShowOpenCaseCraftMenu(id);
			}
		}
		default:
...
_OpenCase(id)
{
	...
	do {
		...
		if (0 >= g_iDropSkinNum)
		{
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_NO_DROP_SKINS");
			_ShowOpenCaseCraftMenu(id);
			return 0;
		}
		...
		if (!(timer < 5 && !succes))
		{
			if (succes)
			{
				...
				{
					color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_DROP_SUCCESS_ALL", g_szName[id], Skin, 100 - wChance);
				}
				else
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_DROP_SUCCESS", Skin, 100 - wChance);
				}
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_DROP_FAIL");
				_ShowOpenCaseCraftMenu(id);
			}
			return 0;
		}
...
_CraftSkin(id)
{
	...
	do {
		...
		{
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_NO_CRAFT_SKINS");
			_ShowOpenCaseCraftMenu(id);
			return 0;
		}
		...
		if (!(timer < 5 && !succes))
		{
			if (succes)
			{
				...
				if (0 < g_iShowDropCraft)
				{
					color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_CRAFT_SUCCESS_ALL", g_szName[id], Skin, 100 - wChance);
				}
				else
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_CRAFT_SUCCESS", Skin, 100 - wChance);
				}
				g_iLastOpenCraft[id] = get_systime(0);
				_ShowOpenCaseCraftMenu(id);
			}
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_CRAFT_FAIL");
				_ShowOpenCaseCraftMenu(id);
			}
			return 0;
		}
...
{
	if (0 <= item < g_iSkinsNum || item == 1 || item == 0)
	{
		return true;
	}
	return false;
...
public market_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		case 33:
		{
			if (g_bUserSell[id])
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_MUST_CANCEL");
				_ShowMarketMenu(id);
			}
			else
			{
		...
		case 34:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_MUST_SELECT");
				_ShowMarketMenu(id);
			}
			else
			{
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_MUST_WAIT", g_iLastPlace[id] - get_systime(0));
				}
				if (1 > g_iUserItemPrice[id])
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_IM_SET_PRICE");
					_ShowMarketMenu(id);
				}
				new wPriceMin;
				new wPriceMax;
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_ITEM_MIN_MAX_COST", wPriceMin, wPriceMax);
					_ShowMarketMenu(id);
					return _MenuExit(menu);
				}
				g_bUserSell[id] = 1;
				...
				color_print(0, "^4%s %L", "[CSGO Remake]", id, "CSGOR_SELL_ANNOUNCE", g_szName[id], Item, g_iUserItemPrice[id]);
			}
		}
		case 35:
		{
			g_bUserSell[id] = 0;
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_CANCEL_SELL");
			_ShowMarketMenu(id);
		}
		default:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_INVALID_SELLER");
				g_bUserSell[index] = 0;
				_ShowMarketMenu(id);
			}
			else
			{
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_DONT_HAVE_ITEM");
					g_bUserSell[index] = 0;
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_NOT_ENOUGH_POINTS", price - g_iUserPoints[id]);
					_ShowMarketMenu(id);
					return 0;
				}
				new szItem[32];
				...
				switch (tItem)
				{
					case 0:
					{
						...
						_SaveData(id);
						color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_X_BUY_Y", g_szName[id], szItem, g_szName[index]);
					}
					case 1:
					{
						g_iUserCases[id]++;
	...
	while (i < g_iSkinsNum)
	{
		...
		if (0 < num)
		{
			...
			switch (type[0])
			{
				...
				default:
				{
					formatex(temp, 63, "\r%s \w| \y%L", szSkin, id, "CSGOR_SM_PIECES", num);
				}
			}
			szItem[0] = i;
			menu_additem(menu, temp, szItem, 0, -1);
...
public item_menu_handler(id, menu, item)
{
	...
	color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_IM_SELECT", szItem);
...
public db_skins_menu_handler(id, menu, item)
{
	...
	switch (g_iMenuType[id])
	{
		case 1:
		{
			...
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRANSFORM", g_iDustForTransform, Skin);
			_SaveData(id);
		}
		case 2:
		{
			...
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_DESTORY", Skin, rest);
			_SaveData(id);
		}
		default:
		{
...
public gift_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		case 2:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_INVALID_TARGET");
				g_iGiftTarget[id] = 0;
				_ShowGiftMenu(id);
			}
			else
			{
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_NOT_ENOUGH_ITEMS");
					g_iGiftItem[id] = -1;
					_ShowGiftMenu(id);
				}
				new gift[16];
				switch (_item)
				{
					case 0:
					{
						...
						color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_SEND_GIFT", gift, g_szName[target]);
						color_print(target, "^4%s^1 %L", "[CSGO Remake]", target, "CSGOR_RECIEVE_GIFT", g_szName[id], gift);
					}
					case 1:
					{
						g_iUserCases[id]--;
					...
					default:
					{
						...
						color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_SEND_GIFT", Skin, g_szName[target]);
						color_print(target, "^4%s^1 %L", "[CSGO Remake]", target, "CSGOR_RECIEVE_GIFT", g_szName[id], Skin);
					}
				}
				g_iGiftTarget[id] = 0;
				g_iGiftItem[id] = -1;
...
public st_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		default:
		{
			...
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_YOUR_TARGET", name);
			_ShowGiftMenu(id);
		}
	}
	return _MenuExit(menu);
...
public si_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		default:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_INVALID_GIFT");
				_SelectItem(id);
			}
			else
			{
				...
				new szItem[32];
				_GetItemName(index, szItem, 31);
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_YOUR_GIFT", szItem);
				_ShowGiftMenu(id);
			}
		}
	}
...
_ShowTradeMenu(id)
{
	...
	{
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_INFO2");
		return 0;
	}
	new temp[64];
...
public trade_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		case 0:
		{
			if (g_bTradeActive[id] || g_bTradeSecond[id])
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_LOCKED");
				_ShowTradeMenu(id);
			}
			else
			{
		...
		case 2:
		{
			...
			else
			{
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_FAIL");
					color_print(target, "^4%s^1 %L", "[CSGO Remake]", target, "CSGOR_TRADE_FAIL");
					_ResetTradeData(id);
				...
				if (!g_bTradeSecond[id])
				{
					color_print(target, "^4%s^1 %L", "[CSGO Remake]", target, "CSGOR_TRADE_INFO1", g_szName[id], szItem);
					color_print(target, "^4%s^1 %L", "[CSGO Remake]", target, "CSGOR_TRADE_INFO2");
				}
				else
				{
					new yItem[32];
					_GetItemName(g_iTradeItem[target], yItem, 31);
					color_print(target, "^4%s %L", "[CSGO Remake]", target, "CSGOR_TRADE_INFO3", g_szName[id], szItem, yItem);
					color_print(target, "^4%s^1 %L", "[CSGO Remake]", target, "CSGOR_TRADE_INFO2");
					g_bTradeAccept[target] = 1;
				}
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_SEND", g_szName[target]);
			}
		}
		case 3:
		{
			...
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_CANCELED");
			_ShowTradeMenu(id);
		}
		default:
		{
...
public tst_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		default:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TARGET_TRADE_ACTIVE", name);
			}
			else
			{
...
public tsi_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		default:
		{
			if (index == g_iUserSellItem[id] && g_bUserSell[id])
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_INVALID_ITEM");
				_SelectTradeItem(id);
			}
			else
			{
				...
				new szItem[32];
				_GetItemName(index, szItem, 31);
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_ITEM", szItem);
				_ShowTradeMenu(id);
			}
		}
	}
...
public clcmd_say_accept(id)
{
	...
	{
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_DONT_HAVE_REQ");
		return 1;
	}
	if (!g_bLogged[sender] || !(0 < sender && 32 >= sender))
	{
		_ResetTradeData(id);
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_INVALID_SENDER");
		return 1;
	}
	if (!g_bTradeActive[sender] && id == g_iTradeTarget[sender])
	{
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_IS_CANCELED");
		_ResetTradeData(id);
		return 1;
	}
	if (g_bTradeAccept[id])
	{
		...
		{
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_FAIL2");
			color_print(sender, "^4%s^1 %L", "[CSGO Remake]", sender, "CSGOR_TRADE_FAIL2");
			_ResetTradeData(id);
		...
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_SUCCESS", tItemsz, sItemsz);
		color_print(sender, "^4%s^1 %L", "[CSGO Remake]", sender, "CSGOR_TRADE_SUCCESS", sItemsz, tItemsz);
		_ResetTradeData(id);
		_ResetTradeData(sender);
	}
	else
	{
		if (!g_bTradeSecond[id])
		{
			...
			g_bTradeSecond[id] = 1;
			_ShowTradeMenu(id);
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TRADE_SELECT_ITEM");
		}
	}
	return 1;
}
...
public clcmd_say_deny(id)
{
	...
	_ResetTradeData(id);
	_ResetTradeData(sender);
	color_print(sender, "^4%s^1 %L", "[CSGO Remake]", sender, "CSGOR_TARGET_REFUSE_TRADE", g_szName[id]);
	color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_YOU_REFUSE_TRADE", g_szName[sender]);
	return 1;
}
...
public games_menu_handler(id, menu, item)
{
	...
	switch (item)
	{
		...
		case 1:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_NOT_ENOUGH_POINTS", g_iRouletteCost - points);
				_ShowGamesMenu(id);
			}
			else
			{
				...
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_ROULETTE_NEXT");
					_ShowGamesMenu(id);
				}
				else
				{
		...
		case 2:
		{
			...
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_JP_CLOSED", get_pcvar_num(c_JackpotTimer));
			}
		}
		default:
		{
...
public tombola_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		case 1:
		{
			...
			if (!g_bTombolaWork)
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_TOMB_NOT_WORK");
			}
			else
			{
				if (uPoints < g_iTombolaCost)
				{
					color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_NOT_ENOUGH_POINTS", g_iTombolaCost - uPoints);
					_ShowTombolaMenu(id);
					return 0;
				}
				...
				g_iTombolaPlayers += 1;
				_SaveData(id);
				color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_TOMB_ANNOUNCE", g_szName[id]);
				_ShowTombolaMenu(id);
			}
		}
		default:
...
public task_TombolaRun(task)
{
	...
	{
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_TOMB_FAIL_REG");
	}
	else
	{
		if (2 > g_iTombolaPlayers)
		{
			color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_TOMB_FAIL_NUM");
		}
		new id;
		...
		do {
			...
			if (0 < id && 32 >= id || !is_user_connected(id))
			{
				...
				else
				{
					...
					color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_TOMB_WINNER", Name, g_iTombolaPrize);
				}
			}
			else
			{
	...
	new Timer[32];
	_FormatTime(Timer, 31, g_iNextTombolaStart);
	color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_TOMB_NEXT", Timer);
	return 0;
}
...
public roulette_menu_handler(id, menu, item)
{
	...
	if (0 <= item <= 2)
	{
		...
		{
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_NOT_ENOUGH_POINTS", g_iUserBetPoints[id] - g_iUserPoints[id]);
			return _MenuExit(menu);
		}
	}
	...
	switch (item)
	{
		...
		case 3:
		{
			color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_INSERT_BET");
...
_RouletteWin(id, multi, announce)
{
	...
	if (0 < announce)
	{
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_ROULETTE_WIN", g_szName[id], g_iUserBetPoints[id]);
	}
	else
	{
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_ROULETTE_WIN_ONE", g_iUserBetPoints[id]);
	}
	_SaveData(id);
	color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_ROULETTE_NEXT");
	return 0;
...
{
	color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_ROULETTE_LOOSE", num);
	_ShowGamesMenu(id);
	return 0;
}
...
public concmd_betpoints(id)
{
	...
	{
		color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_BET_MIN_MAX", 10, 1000);
		client_cmd(id, "messagemode BetPoints");
		return 1;
	}
...
public jackpot_menu_handler(id, menu, item)
{
	...
	switch (index)
	{
		...
		case 2:
		{
			...
			if (!_IsGoodItem(skin))
			{
				color_print(id, "^4%s^1 %L", "[CSGO Remake]", id, "CSGOR_SKINS");
				_ShowJackpotMenu(id);
			}
			else
			{
				...
				color_print(0, "^4%s %L", "[CSGO Remake]", -1, "CSGOR_JP_JOIN", g_szName[id], szItem);
			}
		}
		default:
		{
...
public task_Jackpot(task)
{
	...
	if (1 > size)
	{
		color_print(0, "^4%s %L", "[CSGO Remake]", -1, "CSGOR_JP_NO_ONE");
		_ClearJackpot();
		return 0;
	}
	if (2 > size)
	{
		color_print(0, "^4%s %L", "[CSGO Remake]", -1, "CSGOR_JP_ONLY_ONE");
		new id;
		new k;
	...
	do {
		...
		if (0 < id && 32 >= id || !is_user_connected(id))
		{
			...
			_SaveData(id);
			color_print(0, "^4%s %L", "[CSGO Remake]", -1, "CSGOR_JP_WINNER", g_szName[id]);
		}
		else
		{
...
_ClearJackpot()
{
	...
	color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_JP_NEXT");
	return 0;
}
...
public ev_DeathMsg()
{
	...
	if (equal(szWeapon, "knife", 0))
	{
		...
		color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_KNIFE_KILL", g_szName[killer], 1000);
	}
	g_iDigit[killer]++;
	...
	if (!g_bLogged[killer])
	{
		color_print(killer, "^4%s^1 %L", "[CSGO Remake]", killer, "CSGOR_REGISTER");
		return 0;
	}
	g_iUserKills[killer]++;
	_SaveData(killer);
	...
	if (g_iRanksNum + -1 > g_iUserRank[killer])
	{
		if (ArrayGetCell(g_aRankKills, g_iUserRank[killer] + 1) <= g_iUserKills[killer] < ArrayGetCell(g_aRankKills, g_iUserRank[killer] + 2))
		{
			...
			color_print(0, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_LEVELUP_ALL", g_szName[killer], szRank);
		}
	}
	new rpoints;
	new rchance;
	...
	if (rchance > g_iDropChance)
	{
		...
		switch (r)
		{
			case 1:
			{
				...
				if (0 < g_iDropType)
				{
					color_print(killer, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_REWARD_CASE2");
				}
				else
				{
					color_print(killer, "^4%s^1 %L", "[CSGO Remake]", -1, "CSGOR_REWARD_CASE");
				}
			}
			case 2:
			{
				g_iUserKeys[killer]++;
				_SaveData(killer);
				color_print(killer, "^4%s %L", "[CSGO Remake]", -1, "CSGOR_REWARD_KEY");
			}
			default:
			{
			}
	...
	if (levelup)
	{
		...
		_SaveData(killer);
		color_print(killer, "^4%s^1 %L", "[CSGO Remake]", killer, "CSGOR_RANKUP_BONUS", keys, cases, points);
	}
	return 0;
}
...
public concmd_givepoints(id, level, cid)
{
	...
	if (0 > amount)
	{
		...
		color_print(target, "^4%s^1 %L %L", "[CSGO Remake]", target, "CSGOR_ADMIN_SUB_YOU", g_szName[id], amount, target, "CSGOR_POINTS");
	}
	else
	{
		if (0 < amount)
		{
			...
			color_print(target, "^4%s^1 %L %L", "[CSGO Remake]", target, "CSGOR_ADMIN_ADD_YOU", g_szName[id], amount, target, "CSGOR_POINTS");
		}
		return 1;
	}
	_SaveData(target);
...
public concmd_givecases(id, level, cid)
{
	...
	if (0 > amount)
	{
		...
		color_print(target, "^4%s^1 %L %L", "[CSGO Remake]", target, "CSGOR_ADMIN_SUB_YOU", g_szName[id], amount, target, "CSGOR_CASES");
	}
	else
	{
		if (0 < amount)
		{
			...
			color_print(target, "^4%s^1 %L %L", "[CSGO Remake]", target, "CSGOR_ADMIN_ADD_YOU", g_szName[id], amount, target, "CSGOR_CASES");
		}
		return 1;
	}
	_SaveData(target);
...
public concmd_givekeys(id, level, cid)
{
	...
	if (0 > amount)
	{
		...
		color_print(target, "^4%s^1 %L %L", "[CSGO Remake]", target, "CSGOR_ADMIN_SUB_YOU", g_szName[id], amount, target, "CSGOR_KEYS");
	}
	else
	{
		if (0 < amount)
		{
			...
			color_print(target, "^4%s^1 %L %L", "[CSGO Remake]", target, "CSGOR_ADMIN_ADD_YOU", g_szName[id], amount, target, "CSGOR_KEYS");
		}
		return 1;
	}
	_SaveData(target);
...
public concmd_givedusts(id, level, cid)
{
	...
	if (0 > amount)
	{
		...
		color_print(target, "^4%s^1 %L %L", "[CSGO Remake]", target, "CSGOR_ADMIN_SUB_YOU", g_szName[id], amount, target, "CSGOR_DUSTS");
	}
	else
	{
		if (0 < amount)
		{
			...
			color_print(target, "^4%s^1 %L %L", "[CSGO Remake]", target, "CSGOR_ADMIN_ADD_YOU", g_szName[id], amount, target, "CSGOR_DUSTS");
		}
		return 1;
	}
	_SaveData(target);
...
public concmd_setrank(id, level, cid)
{
	...
	color_print(target, "^4%s^1 %L", "[CSGO Remake]", target, "CSGOR_ADMIN_SET_RANK", g_szName[id], szRank);
	return 1;
}
...
public concmd_giveskins(id, level, cid)
{
	...
	if (0 > amount)
	{
		...
		color_print(target, "^4%s^1 %L x ^3%s", "[CSGO Remake]", target, "CSGOR_ADMIN_SUB_YOU", g_szName[id], amount, szSkin);
	}
	else
	{
		if (0 < amount)
		{
			...
			color_print(target, "^4%s^1 %L x ^3%s", "[CSGO Remake]", target, "CSGOR_ADMIN_ADD_YOU", g_szName[id], amount, szSkin);
		}
		return 1;
	}
	_SaveData(target);
